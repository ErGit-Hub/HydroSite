...
Помимо ручной выкладки, по расписанию (или по кнопке «Run workflow» на вкладке
Actions) отрабатывает `.github/workflows/telegram-news.yml`. В самом файле стоит
`cron: '*/30 * * * *'`, но это не гарантия — `schedule`-триггеры GitHub Actions
официально «best effort», и на практике для этого репозитория реальный разброс
между прогонами обычно 2-5 часов, а не 30 минут (проверено по истории запусков
2026-09-06 — 2026-09-09). Это ограничение платформы, не баг воркфлоу — можно
починить внешним планировщиком, который дёргал бы `workflow_dispatch` по API
